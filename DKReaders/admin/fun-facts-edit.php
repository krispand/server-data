<?
 ...
 require('config/upload.php');
	...
	$funCols = array('funfact_auto_id','funfact_title','funfact_image','funfact_region_id','funfact_startdate','funfact_enddate','funfact_status');
	$searchStr = "";
 	$sql = 'CALL funcGetRecords(\'funfacts\',\''.implode(',',$funCols).'\',\''.addslashes($searchStr).'\',\''.$_REQUEST['id'].'\','.$_SESSION['login_region'].')';
...
if($result){
	...
	foreach ($funCols as $cols) {
		// ### get col name save data to array
		$funArray[$cols] = $row->$cols;
	}
}
 ...
 // ### Start insert of record
	...
	$enddate= $arraydate[1];
 	...
 	if ($_FILES['funfact_image']['tmp_name']!=''){
		...
		$imagename = $funArray['funfact_image'];
	}

	 
	 $sql ='CALL funcSaveFunFact(\'edit\',\''.addslashes($_POST['funfact_title']).'\',\''.$imagename.'\','.intNull($_POST['book_region_id'],1).',\''.strToDBDate($startdate).'\',\''.strToDBDate($enddate).'\',\''.$_POST['funfact_status'].'\','.$funArray['funfact_auto_id'].')';
...
<div class="controls">
	...
	<input id="funfact_title" name="funfact_title" type="text"  style="width:95%" maxlength="220" value="<?=$funArray['funfact_title']?>"/>
...
<div style="max-width: 200px; max-height: 150px; line-height: 20px;" class="fileupload-preview fileupload-exists thumbnail"><a href="<?= (strlen($funArray['funfact_image'])>3)? '../assets/funfacts/'.$funArray['funfact_image'] : 'http://www.placehold.it/200x150/EFEFEF/AAAAAA&amp;text=no+image'?>"  target="_blank"><img alt="" src="<?= (strlen($funArray['funfact_image'])>3)? '../assets/funfacts/'.$funArray['funfact_image'] : 'http://www.placehold.it/200x150/EFEFEF/AAAAAA&amp;text=no+image'?>"></a></div>
<div><span class="btn btn-file"><span class="fileupload-new">Select image</span><span class="fileupload-exists">Change</span>
<input type="file" id="funfact_image" name="funfact_image" class="default"></span></div>
</div></div></div>
...
<select name="book_region_id" id="book_region_id" class="chzn-select" data-placeholder="Select Region" tabindex="1">
    ...
    <option value="<?=$record['region_auto_id']?>" <?= ($record['region_auto_id'] == $funArray['funfact_region_id'])? 'selected':''?>><?=$record['region_code']?></option>
    <? }?>
</select>
	</div>
</div></div>
<? } else {?>
<input name="book_region_id" type="hidden" value="<?=$funArray['funfact_region_id']?>" />
<? }?>
<?
$daterange =date('d/m/Y',strtotime($funArray['funfact_startdate'])).' to '.date('d/m/Y',strtotime($funArray['funfact_enddate']));
?>
<div class="control-group">
...
<input id="funfact_daterange" name="funfact_daterange" type="text" class=" m-ctrl-medium " value="<?=$daterange?>"/>
</div>
</div>
</div>
</div>
...
<option value="DRAFT" <?= ($funArray['funfact_status'] =='DRAFT')? 'selected':''?>>Draft</option>
<option value="LIVE" <?= ($funArray['funfact_status'] =='LIVE')? 'selected':''?>>Live</option>
</select>
</div></div>
</div>
