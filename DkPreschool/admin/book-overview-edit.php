<?
 ...
 $bookCols = array('book_auto_id','book_title','book_author_name','book_image','book_epub','book_sample_epub','book_reader_level','book_region_id','book_short_desc','book_long_desc','book_new','book_free','book_onoffer','book_feature','book_readaloud','book_publication_date','book_apple_purchase_id','book_android_purchase_id','book_topics','book_status');
 
 
 	$searchStr = "";
 	$sql = 'CALL funcGetRecords(\'books\',\''.implode(',',$bookCols).'\',\''.addslashes($searchStr).'\',\''.$_REQUEST['id'].'\','.$_SESSION['login_region'].')';
	...
	if($result){
		...
		foreach ($bookCols as $cols) {
			// ### get col name save data to array
			$bookArray[$cols] = $row->$cols;
		}
	}
	//die();
 if($_POST['book_title']){
 // ### Start insert of record
 //echo "Inside Title".$_POST['book_title'];
 //echo " Temp Name". $_FILES['book_epub']['tmp_name'];
	$tags = '';
	...
	if($_POST['book_topics']){
 	...
 	if ($_FILES['book_image']['tmp_name']!=''){
		...
		$imagename = $bookArray['book_image'];
	}

	if ($_FILES['book_epub']['tmp_name']!=''){
			// ### upload epub file
			$filePath = '../assets/books/epubs/';
		...
		$fileEPubname = $bookArray['book_epub'];
	}
	
	if ($_FILES['book_sample_epub']['tmp_name']!=''){
			// ### upload epub file
			$filePath = '../assets/books/epubsamples/';
		...
		$fileEPubSampname = $bookArray['book_sample_epub'];
	}
	 
	 
	 $sql ='CALL funcSaveBook(\'edit\',\''.addslashes($_POST['book_title']).'\',\''.$_POST['book_author_name'].'\',\''.$imagename.'\',\''.$fileEPubname.'\',\''.$fileEPubSampname.'\','.intNull($_POST['book_reader_level'],1).','.intNull($_POST['book_region_id'],1).',\''.addslashes($_POST['book_short_desc']).'\',\''.addslashes($_POST['book_long_desc']).'\','.intNull($_POST['book_onoffer'],0).','.intNull($_POST['book_new'],0).','.intNull($_POST['book_free'],0).','.intNull($_POST['book_feature'],0).','.intNull($_POST['book_readaloud'],0).',\''.strToDBDate($_POST['book_publication_date']).'\',\''.$_POST['book_apple_purchase_id'].'\',\''.$_POST['book_android_purchase_id'].'\',\''.addslashes($tags).'\',\''.$_POST['book_status'].'\','.$_REQUEST['id'].')';
	$mysqli = connectMySqli();
	$result = $mysqli->query($sql);
...
<div class="controls">
	<div id="book_title-e" class="span4"><div class="icon-exclamation-sign"></div><input id="book_title" name="book_title" type="text" style="width:95%" maxlength="220" value="<?=$bookArray['book_title']?>"/></div>
</div></div>
...
<div class="controls">
<div class="span4">
<input id="book_author_name" name="book_author_name" type="text" style="width:95%"  maxlength="220" value="<?=$bookArray['book_author_name']?>"/>
</div>
</div></div>
...
<div style="max-width: 200px; max-height: 150px; line-height: 20px;" class="fileupload-preview fileupload-exists thumbnail"><a href="<?= (strlen($bookArray['book_image'])>3)? '../assets/books/images/'.$bookArray['book_image'] : 'http://www.placehold.it/200x150/EFEFEF/AAAAAA&amp;text=no+image'?>"  target="_blank"><img alt="" src="<?= (strlen($bookArray['book_image'])>3)? '../assets/books/images/'.$bookArray['book_image'] : 'http://www.placehold.it/200x150/EFEFEF/AAAAAA&amp;text=no+image'?>"></a></div>
<div><span class="btn btn-file"><span class="fileupload-new">Select image</span><span class="fileupload-exists">Change</span>
<input type="file" id="book_image" name="book_image" class="default"></span></div>
</div></div></div>


<div class="control-group">
<label for="book_epub" class="control-label">Upload Book (ePub file)</label>
<div class="controls"><div data-provides="fileupload" class="fileupload fileupload-exists">
<div class="input-append"><div class="uneditable-input">
<i class="icon-file fileupload-exists"></i>
<span class="fileupload-preview"><a href="../book.php?id=<?=$bookArray['book_auto_id']?>&t=607954b7836ee908b463bc61b23d5e14" target="_blank"><?=$bookArray['book_epub']?></a></span>
</div>
...
<div class="input-append"><div class="uneditable-input">
<i class="icon-file fileupload-exists"></i>
<span class="fileupload-preview"><a href="../book.php?id=<?=$bookArray['book_auto_id']?>&sample=1&t=607954b7836ee908b463bc61b23d5e14" target="_blank"><?=$bookArray['book_sample_epub']?></a></span>
</div>
<span class="btn btn-file"><span class="fileupload-new">Select file</span>
...
?>
<label for="book_reader_level" class="control-label">Series Name</label>
<div class="controls">
<div id="book_region_level-e" class="span4"><div class="icon-exclamation-sign"></div>
    ...
    <option value="">Select...</option>
   	<? 	foreach  ($readerArray as $record) { ?>
    	<option value="<?=$record['readerlevel_level']?>" <?= ($record['readerlevel_level'] == $bookArray['book_reader_level'])? 'selected':''?>><?=$record['readerlevel_name']?></option>
    <? }?>
    </select></div>
</div></div>
...
<select name="book_region_id" id="book_region_id" class="chzn-select " data-placeholder="Select Region" tabindex="1">
    ...
    <option value="<?=$record['region_auto_id']?>" <?= ($record['region_auto_id'] == $bookArray['book_region_id'])? 'selected':''?>><?=$record['region_code']?></option>
    <? }?>
</select></div>
</div></div>
<? } else {?>
<input name="book_region_id" type="hidden" value="<?=$bookArray['book_region_id']?>" />
<? }?>
...
<textarea id="book_short_desc" name="book_short_desc" style="width:95%" rows="2" maxlength="150"><?=$bookArray['book_short_desc']?></textarea></div>
</div></div>
...
<textarea id="book_long_desc"  name="book_long_desc" style="width:95%" rows="6" maxlength="3000" ><?=$bookArray['book_long_desc']?></textarea></div>
</div></div>

<div class="control-group">
<label class="control-label">Book Criteria</label>
<div class="controls">
<label class="checkbox"><input name="book_new" type="checkbox" value="1" <?= ($bookArray['book_new']==1)? 'checked':''?>/> New Book</label>
<label class="checkbox"><input name="book_free" type="checkbox" value="1" <?= ($bookArray['book_free']==1)? 'checked':''?>/> Free Book</label>
<label class="checkbox"><input name="book_feature" type="checkbox" value="1" <?= ($bookArray['book_feature']==1)? 'checked':''?>/> Popular Book</label><!-- Feature to Popular -->
<!--<label class="checkbox"><input name="book_onoffer" type="checkbox" value="1" <?= ($bookArray['book_onoffer']==1)? 'checked':''?>/> On Offer</label>
<label class="checkbox"><input name="book_readaloud" type="checkbox" value="1" <?= ($bookArray['book_readaloud']==1)? 'checked':''?>/> Read Aloud</label>--><!-- Commented for PreSchool -->
</div></div>
...
<input name="book_publication_date" id="book_publication_date" type="text" style="width:95%" placeholder="" data-mask="99/99/9999"  value="<?=date('d/m/Y',strtotime($bookArray['book_publication_date']))?>"></div>
<span class="help-inline">dd/mm/yyyy</span>
</div></div>

<div class="control-group">
<label class="control-label">Apple In App Purchase ID</label>
<div class="controls"><div class="span4"><input name="book_apple_purchase_id" type="text" style="width:95%" maxlength="150" value="<?=$bookArray['book_apple_purchase_id']?>"/></div>
</div></div>

<div class="control-group">
<label class="control-label">Andriod In App Purchase ID</label>
<div class="controls"><div class="span4"><input name="book_android_purchase_id" type="text" style="width:95% " maxlength="150" value="<?=$bookArray['book_android_purchase_id']?>"/></div>
</div></div>
...
<div class="control-group"><label class="control-label">Enter keywords associated to book</label>
    <div class="controls">
    <input name="book_topics" id="book_topics" type="hidden" class="tags" value="<?=str_replace('|','',$bookArray['book_topics'])?>" />
    </div>
</div>
<!-- END FORM-->
...
<div class="controls">
<div class="span4">
<select name="book_status" class="span4 chzn-select" tabindex="1">
<option value="DRAFT" <?= ($bookArray['book_status'] =='DRAFT')? 'selected':''?>>Draft</option>
<option value="PUBLISHED" <?= ($bookArray['book_status'] =='PUBLISHED')? 'selected':''?>>Published</option>
<option value="UNPUBLISHED" <?= ($bookArray['book_status'] =='UNPUBLISHED')? 'selected':''?>>Unpublished</option>
</select>
</div></div>
</div>
