 ...
 if(isset($_GET['delete_id']))
{
	...
	echo $bookfilename,$booksamplefilename;
	$bookpath =$serverName.'../assets/books/epub-13/';
	$booksamplepath = $serverName.'../assets/books/epub-samples/';
	if(strlen($bookfilename)>0)
	{
		echo "Actual file exist";
		echo $bookpath.$bookfilename;
		if (file_exists($bookpath.$bookfilename)) 
		{
			if(unlink($bookpath.$bookfilename))
			{
	...
	if(strlen($booksamplefilename)>0)
	{
		echo "Sample file exist";
		if (file_exists($booksamplepath.$booksamplefilename)) 
		{
			if(unlink($booksamplepath.$booksamplefilename))
			{
				//echo "Deleted Successfully";
			}
			else
		...
		//unlink($booksamplepath.$booksamplefilename);
	}
	//die();
	$sql = "CALL funcDeleteBookFromStore(".$bookID.")";
	//echo $sql;
	//die();
	$mysqli1 = connectMySqli();
...
function delete_id(id)
{
     ...
     {
     	//alert('book-overview.php?delete_id='+id);
		location.href='book-overview.php?delete_id='+id;
     }
}
</script>
</head>
...
<? 	foreach  ($dcArray as $record) { ?>
	<tr class="">
        ...
        <td><a href="javascript:delete_id(<?php echo $record['book_auto_id'] ?>)">Delete</a></td>
	</tr>
<? } ?>
</tbody>
</table>
