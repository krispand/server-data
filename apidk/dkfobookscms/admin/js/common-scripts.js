...
function funcDelete(strtype,id,strtitle){
	var del=confirm("Are you sure you want to Delete '"+strtitle+"' ?");
	if (del==true) {
	// ajax call to delete record then refresh current page when done
	$.ajax({
		type: 'GET',
		url: 'remove.php?id='+id+'&type='+ strtype 
	}).done(function(){
		$('#'+strtype+'_'+id).remove();
	});
  } 
