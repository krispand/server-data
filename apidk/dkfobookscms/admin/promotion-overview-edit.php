<?
 ...
 $promoCols = array('promo_title','promo_image','promo_image_lrg','promo_region_id','promo_book_ids','promo_type','promo_startdate','promo_enddate','promo_status');
	...
	$searchStr = "";
 	$sql = 'CALL funcGetRecords(\'promos\',\''.implode(',',$promoCols).'\',\''.addslashes($searchStr).'\',\''.$_REQUEST['id'].'\','.$_SESSION['login_region'].')';
	...
	if($result){
		...
		foreach ($promoCols as $cols) {
			// ### get col name save data to array
			$promoArray[$cols] = $row->$cols;
		}
	}
 ...
 // ### Start insert of record
	...
	$enddate= $arraydate[1];
	 ...
	 if ($_FILES['promo_image']['tmp_name']!=''){
		...
		$imagename = $promoArray['promo_image'];
	}
	 	...
	 	if ($_FILES['promo_image_lrg']['tmp_name']!=''){
			...
			$imagelrgname = $promoArray['promo_image_lrg'];
		}
	

	 
	 $sql ='CALL funcSavePromo(\'edit\',\''.addslashes($_POST['promo_title']).'\',\''.$imagename.'\',\''.$imagelrgname.'\','.intNull($_POST['book_region_id'],1).',\''.$bookids.'\','.$type.',\''.strToDBDate($startdate).'\',\''.strToDBDate($enddate).'\',\''.$_POST['promo_status'].'\','.$_REQUEST['id'].')';
...
<div id="promo_title-e" class="span4"><div class="icon-exclamation-sign"></div>
	<input id="promo_title" name="promo_title" type="text" style="width:95%" maxlength="220" value="<?=$promoArray['promo_title']?>"/>
    </div>
</div></div>
...
<div class="controls">
<div data-provides="fileupload" class="fileupload fileupload-exists" data-name="promo-image" required>
<div style="width: 200px; height: 150px;" class="fileupload-new thumbnail"></div>
<div style="max-width: 200px; max-height: 150px; line-height: 20px;" class="fileupload-preview fileupload-exists thumbnail"><a href="<?= (strlen($promoArray['promo_image'])>3)? '../assets/promos/'.$promoArray['promo_image'] : 'http://www.placehold.it/200x150/EFEFEF/AAAAAA&amp;text=no+image'?>"  target="_blank"><img alt="" src="<?= (strlen($promoArray['promo_image'])>3)? '../assets/promos/'.$promoArray['promo_image'] : 'http://www.placehold.it/200x150/EFEFEF/AAAAAA&amp;text=no+image'?>"></a></div>
<div><span class="btn btn-file"><span class="fileupload-new">Select image</span><span class="fileupload-exists">Change</span>
<input type="file" id="promo_image" name="promo_image" class="default"></span></div>
</div></div></div>
...
<div class="controls">
<div data-provides="fileupload" class="fileupload fileupload-exists" data-name="promo-image-lrg" required>
<div style="width: 200px; height: 150px;" class="fileupload-new thumbnail"></div>
<div style="max-width: 200px; max-height: 150px; line-height: 20px;" class="fileupload-preview fileupload-exists thumbnail"><a href="<?= (strlen($promoArray['promo_image_lrg'])>3)? '../assets/promos/'.$promoArray['promo_image_lrg'] : 'http://www.placehold.it/200x150/EFEFEF/AAAAAA&amp;text=no+image'?>"  target="_blank"><img alt="" src="<?= (strlen($promoArray['promo_image_lrg'])>3)? '../assets/promos/'.$promoArray['promo_image_lrg'] : 'http://www.placehold.it/200x150/EFEFEF/AAAAAA&amp;text=no+image'?>"></a></div>
<div><span class="btn btn-file"><span class="fileupload-new">Select image</span><span class="fileupload-exists">Change</span>
<input type="file" id="promo_image_lrg" name="promo_image_lrg" class="default"></span></div>
</div></div></div>
...
<select name="book_region_id" id="book_region_id" class="chzn-select" data-placeholder="Select Region" tabindex="1">
    ...
    <option value="<?=$record['region_auto_id']?>" <?= ($record['region_auto_id'] == $promoArray['promo_region_id'])? 'selected':''?>><?=$record['region_code']?></option>
    <? }?>
</select>
</div>
</div></div>
<? } else {?>
<input name="book_region_id" type="hidden" value="<?=$promoArray['promo_region_id']?>" />
<? }?>
...
?>
<?
if(is_numeric(strpos($promoArray['promo_book_ids'],','))===true){
	$bookids = explode(',',$promoArray['promo_book_ids']);
	$bookids = '|'.implode('|,|',$bookids).'|';
} else {
	$bookids = '|'.$promoArray['promo_book_ids'].'|';
}
?>

<div class="control-group">
    <label class="control-label">Select Books</label>
    <div class="controls">
        ...
        <select id="promo_book_ids" name="promo_book_ids[]" data-placeholder="Add Book" class="chzn-select" multiple="multiple" tabindex="6" style="display:none">
            <option value=""></option>
               	<? 	foreach  ($bookArray as $record) { ?>
    				<option value="<?=$record['book_auto_id']?>" class="bkms-<?=$record['book_region_id']?>" <?= (is_numeric(strpos($bookids,'|'.$record['book_auto_id'].'|'))===true)? 'selected':''?>><?=$record['book_title']?></option>
    			<? }?>
        </select></div>
    </div>
</div>
...
<?
$daterange =date('d/m/Y',strtotime($promoArray['promo_startdate'])).' to '.date('d/m/Y',strtotime($promoArray['promo_enddate']));
?>
<div class="control-group">
<label class="control-label">Promotion Start/End Date</label>
<div class="controls">
<div id="promo_daterange-e"><div class="icon-exclamation-sign"></div>
<div class="input-prepend">
<span class="add-on"><i class="icon-calendar"></i></span>
<input id="promo_daterange" name="promo_daterange" type="text" class=" m-ctrl-medium" value="<?=$daterange?>" />
</div>
</div>
</div>
</div>
...
<option value="DRAFT" <?= ($promoArray['promo_status'] =='DRAFT')? 'selected':''?>>Draft</option>
<option value="LIVE" <?= ($promoArray['promo_status'] =='LIVE')? 'selected':''?>>Live</option>
</select>
</div></div>
</div>
   ...
   <script language="javascript" type="text/javascript">
 		...
 		$().ready(function() {
			...
			<? }?>
				...
				var rid = '.bkms-<?=$promoArray['promo_region_id']?>';
				
			$('.chzn-results li.active-result').addClass('result-selected')
			$('.chzn-results li.active-result'+rid).removeClass('result-selected');
		
			//	console.log($('.chzn-results li.active-result'+rid));
